...
int main(int, char **)
{
    ...
    mtrand Rand(myrank); //seed the random number generator
...
#pragma omp parallel shared(grid)
    {
        ...
        mtrand Rand(omp_rank); //seed the random number generator in each thread
        if (myrank == 0 && omp_rank == 0)
            printf("Total node number: %d. Total local thread number: %d.\n", proc_num, omp_size);
...
#pragma omp parallel for reduction(+ : local_E_i) //calculate the total energy of the spin configuration on the local node
...
#pragma omp parallel for reduction(+ : local_E_f) //calculate the total energy of the new configuration
        for (int i = halo; i < (grid.xsize - halo); i++)
        {
            for (int j = halo; j < (grid.ysize - halo); j++)
            {
